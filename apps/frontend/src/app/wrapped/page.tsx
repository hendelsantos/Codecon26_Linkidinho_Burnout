...
// Meses disponíveis — últimos 6 + total
function buildMonthOptions() {
  const opts: { value: string; label: string }[] = [{ value: "", label: "Todo o histórico" }];
  const now = new Date();
  for (let i = 0; i < 6; i++) {
...
function EquivCard({ emoji, text }: { emoji: string; text: string }) {
  return (
    <motion.div
      className="flex items-start gap-3 rounded-[20px] border border-white/8 bg-black/20 px-4 py-3"
...
export default function WrappedPage() {
  ...
  const load = useCallback(async (month: string) => {
    ...
    try {
      const w = await api.getWrapped(token, month || undefined);
      setData(w);
    } catch {
      setError("Nenhum check-in registrado neste período.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  function buildEquivalencias(d: Wrapped) {
    const eq = d.equivalencias;
    const lines: { emoji: string; text: string }[] = [];
    if (d.meetings_total > 0) {
      lines.push({ emoji: "🎬", text: `${d.meetings_total} reuniões inúteis = assistir Titanic ${eq.titanic_meetings} vezes — com mais sofrimento.` });
    }
    if (d.traffic_hours > 0) {
      lines.push({ emoji: "📺", text: `${d.traffic_hours}h no trânsito = ${eq.netflix_hours}h de Netflix que você não assistiu.` });
    }
    if (d.coffees_total > 0) {
      lines.push({ emoji: "🫗", text: `${d.coffees_total} cafés = ${eq.coffees_litros}L de café. Seu coração é forte.` });
    }
    if (d.buzzwords_total > 0) {
      lines.push({ emoji: "🎵", text: `${d.buzzwords_total} buzzwords aguentadas = ${eq.buzzwords_musicas} músicas corporativas sem sentido.` });
    }
    if (d.bathroom_revenue_reais > 0) {
      lines.push({ emoji: "🚽", text: `R$${d.bathroom_revenue_reais} gerados no banheiro. O mercado é seu oyster.` });
    }
    return lines;
  }
  ...
  if (error || !data) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center gap-4 px-5">
        ...
        {/* Seletor de mês mesmo na tela de erro */}
        <div className="flex flex-wrap justify-center gap-2 mt-4">
          {MONTH_OPTIONS.map((m) => (
            <button
              key={m.value}
              onClick={() => setSelectedMonth(m.value)}
              className={`rounded-full border px-3 py-1.5 text-xs font-medium transition-all ${selectedMonth === m.value ? "border-violet/60 bg-violet/20 text-violet" : "border-white/10 bg-white/5 text-slate-400 hover:text-white"}`}
            >
              {m.label}
            </button>
          ))}
  ...
  const equivalencias = buildEquivalencias(data);
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-5 py-12">
      ...
      >
        ...
        {data.period !== "total" && (
          <p className="mt-1 text-xs text-violet/70">Período: {data.period}</p>
        )}
      ...
      {equivalencias.length > 0 && (
        ...
        >
          <p className="mb-4 text-xs uppercase tracking-[0.3em] text-muted">Para você ter uma ideia</p>
          <div className="space-y-3">
            {equivalencias.map((e) => (
              <EquivCard key={e.text} emoji={e.emoji} text={e.text} />
            ))}
          </div>
        </motion.div>
      )}
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <button
          onClick={() => {
            const periodLabel = data.period !== "total" ? `em ${data.period}` : "no total";
            const text = `${data.profile.emoji} Meu Burny Wrapped ${periodLabel}: ${data.checkins_total} check-ins, ${data.coffees_total} cafés, ${data.meetings_total} reuniões inúteis. Score médio: ${data.burny_score_avg} 🔥 #BurnyOut`;
